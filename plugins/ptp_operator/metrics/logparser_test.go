...
var ptp4lConfigBC = &ptp4lconf.PTP4lConfig{
	Name:    "ptp4l.0.config",
	Profile: "grandmaster",
	Interfaces: []*ptp4lconf.PTPInterface{
		{
...
func TestExtractPTP4lEventState(t *testing.T) {
	tests := []struct {
		...
		// Dual follower
		{
			name:          "dual 1 -- INITIALIZING to LISTENING",
			logLine:       "ptp4l[415036.213]: [ptp4l.0.config:5] port 1 (ens1f0): INITIALIZING to LISTENING on INIT_COMPLETE",
			expectedPort:  1,
			expectedRole:  types.LISTENING,
			expectedState: ptp.FREERUN,
		},
		{
			name:          "dual 2 -- INITIALIZING to LISTENING",
			logLine:       "ptp4l[415036.243]: [ptp4l.0.config:5] port 2 (ens1f1): INITIALIZING to LISTENING on INIT_COMPLETE",
			expectedPort:  2,
			expectedRole:  types.LISTENING,
			expectedState: ptp.FREERUN,
		},
		{
			name:          "dual 3 -- LISTENING to UNCALIBRATED",
			logLine:       "ptp4l[415037.374]: [ptp4l.0.config:5] port 1 (ens1f0): LISTENING to UNCALIBRATED on RS_SLAVE",
			expectedPort:  1,
			expectedRole:  types.FAULTY,
			expectedState: ptp.HOLDOVER,
		},
		{
			name:          "dual 4 -- UNCALIBRATED to SLAVE",
			logLine:       "ptp4l[415037.689]: [ptp4l.0.config:5] port 1 (ens1f0): UNCALIBRATED to SLAVE on MASTER_CLOCK_SELECTED",
			expectedPort:  1,
			expectedRole:  types.SLAVE,
			expectedState: ptp.FREERUN,
		},
		// port ens1f0 down then up
		{
			name:          "dual 5 -- SLAVE to FAULTY",
			logLine:       "ptp4l[415183.810]: [ptp4l.0.config:5] port 1 (ens1f0): SLAVE to FAULTY on FAULT_DETECTED (FT_UNSPECIFIED)",
			expectedPort:  1,
			expectedRole:  types.FAULTY,
			expectedState: ptp.HOLDOVER,
		},
		{
			name:          "dual 6 -- LISTENING to UNCALIBRATED",
			logLine:       "ptp4l[415183.848]: [ptp4l.0.config:5] port 2 (ens1f1): LISTENING to UNCALIBRATED on RS_SLAVE",
			expectedPort:  2,
			expectedRole:  types.FAULTY,
			expectedState: ptp.HOLDOVER,
		},
		{
			name:          "dual 7 -- UNCALIBRATED to SLAVE",
			logLine:       "ptp4l[415183.995]: [ptp4l.0.config:5] port 2 (ens1f1): UNCALIBRATED to SLAVE on MASTER_CLOCK_SELECTED",
			expectedPort:  2,
			expectedRole:  types.SLAVE,
			expectedState: ptp.FREERUN,
		},
		{
			name:          "dual 8 -- FAULTY to LISTENING",
			logLine:       "ptp4l[415204.649]: [ptp4l.0.config:5] port 1 (ens1f0): FAULTY to LISTENING on INIT_COMPLETE",
			expectedPort:  1,
			expectedRole:  types.LISTENING,
			expectedState: ptp.FREERUN,
		},
		// all masters down
		{
			name:          "dual 9 -- LISTENING to UNCALIBRATED",
			logLine:       "ptp4l[415205.789]: [ptp4l.0.config:5] port 1 (ens1f0): LISTENING to UNCALIBRATED on RS_SLAVE",
			expectedPort:  1,
			expectedRole:  types.FAULTY,
			expectedState: ptp.FREERUN,
		},
		{
			name:          "dual 10 -- SLAVE to LISTENING",
			logLine:       "ptp4l[415205.789]: [ptp4l.0.config:5] port 2 (ens1f1): SLAVE to LISTENING on RS_PASSIVE",
			expectedPort:  2,
			expectedRole:  types.LISTENING,
			expectedState: ptp.HOLDOVER,
		},
		{
			name:          "dual 11 -- UNCALIBRATED to SLAVE",
			logLine:       "ptp4l[415205.975]: [ptp4l.0.config:5] port 1 (ens1f0): UNCALIBRATED to SLAVE on MASTER_CLOCK_SELECTED",
			expectedPort:  1,
			expectedRole:  types.SLAVE,
			expectedState: ptp.FREERUN,
		},
		{
			name:          "dual 12 -- SLAVE to LISTENING",
			logLine:       "ptp4l[415277.729]: [ptp4l.0.config:5] port 1 (ens1f0): SLAVE to LISTENING on ANNOUNCE_RECEIPT_TIMEOUT_EXPIRES",
			expectedPort:  1,
			expectedRole:  types.LISTENING,
			expectedState: ptp.HOLDOVER,
		},
	...
	for _, tc := range tests {
		...
		t.Run(tc.name, func(t *testing.T) {
			portID, role, clockState := metrics.TestFuncExtractPTP4lEventState(tc.logLine, ptp4lConfigBC)

			if portID != tc.expectedPort {
				assert.Equal(t, tc.expectedPort, portID, fmt.Sprintf("with portID = %d; want %d", portID, tc.expectedPort))
			}
			if role != tc.expectedRole {
				assert.Equal(t, tc.expectedRole, role, fmt.Sprintf("with role = %v; want %v", role, tc.expectedRole))
			}
			if clockState != tc.expectedState {
				assert.Equal(t, tc.expectedPort, clockState, fmt.Sprintf("with state = %v; want %v", clockState, tc.expectedState))
			}
		})
	}
}
...
func TestParsePTP4l(t *testing.T) {
	tests := []struct {
		...
		expectedMasterState ptp.SyncState
	...
	}{
		{
			...
			ptpStats:            stats.PTPStats{},
			expectedMasterState: ptp.FREERUN,
			expectedError:       false,
		},
		{
		...
		},
		{
			...
			configName:  "ptp4l.0.config",
			profileName: "test-profile",
			output:      "ptp4l[72444.514]: [ptp4l.0.config:5] port 1 (ens2f0): LISTENING to SLAVE",
			fields:      []string{"ptp4l", "1646672953", "ptp4l.0.config", "port", "1", "(ens2f0)", "LISTENING to SLAVE"},
			ptpInterface: ptp4lconf.PTPInterface{
				Name: "ens2f0",
			},
			ptp4lCfg: &ptp4lconf.PTP4lConfig{
			...
			t.Run(tt.name, func(t *testing.T) {
				if followers > 1 {
					for i := 0; i < followers; i++ {
						if tt.ptp4lCfg.Interfaces[i].Role == types.MASTER {
							tt.ptp4lCfg.Interfaces[i].UpdateRole(types.LISTENING)
						}
					}
				}
				metrics.SetMasterOffsetSource(tt.processName)
				ptpEventManager.Stats[types.ConfigName(tt.configName)] = make(stats.PTPStats)
				tt.ptpStats = ptpEventManager.GetStats(types.ConfigName(tt.configName))
				tt.ptpStats.CheckSource(metrics.ClockRealTime, configName, tt.processName)
				tt.ptpStats.CheckSource(metrics.MasterClockType, configName, tt.processName)
				tt.ptpStats[metrics.MasterClockType].SetLastSyncState(tt.lastSyncState)
				ptpEventManager.ParsePTP4l(tt.processName, tt.configName, tt.profileName, tt.output, tt.fields, tt.ptpInterface, tt.ptp4lCfg, tt.ptpStats)
				if tt.expectedStateChange {
					tt.ptpStats = ptpEventManager.GetStats(types.ConfigName(tt.configName))
					assert.Equal(t, tt.expectedMasterState, tt.ptpStats[metrics.MasterClockType].LastSyncState(), fmt.Sprintf("%s-followers(%d) state = %v; want %v", tt.name, followers, tt.ptpStats[metrics.MasterClockType].LastSyncState(), tt.expectedMasterState))
				}
			})
		}
	}
