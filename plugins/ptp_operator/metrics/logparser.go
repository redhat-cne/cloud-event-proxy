...
func (p *PTPEventManager) ParseTBCLogs(processName, configName, output string, fields []string,
	...
	if clockState.State != lastClockState && clockState.State != "" { // publish directly here
		...
		UpdateSyncStateMetrics(ts2phcProcessName, aliasValue, ptpStats[masterType].LastSyncState())
		...
		UpdateSyncStateMetrics(ptp4lProcessName, aliasValue, ptpStats[masterType].LastSyncState())
