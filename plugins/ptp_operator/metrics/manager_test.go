...
func TestListHAProfilesWith(t *testing.T) {
	...
	result := manager.ListHAProfilesWith("profile2")
	assert.ElementsMatch(t, []string{"profile1", "profile2"}, result)

	// Case 3: Not found
	result = manager.ListHAProfilesWith("unknown")
	assert.Equal(t, len(result), 0)

	// Case 4: Empty input
	result = manager.ListHAProfilesWith(" ")
	assert.Equal(t, len(result), 0)
	...
	result = manager.ListHAProfilesWith("profile1")
	assert.Equal(t, len(result), 0)
}
