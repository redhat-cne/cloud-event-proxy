...
func (p *PTPEventManager) LoadFromStore(storePath string) (int64, error) {
	contents, err := os.ReadFile(filepath.Join(storePath, metricFilename))
	...
	for configName, portAndRole := range savedMetrics.PortRoles {
		for port, role := range portAndRole {
			...
			ptpInterface, err := ptpConf.ByInterface(port)
			if err != nil {
				continue
			}
...
func (p *PTPEventManager) saveToStore() error {
	...
	if err != nil {
		err = fmt.Errorf("failed to marshall save data fron current data: %w", err)
	}
	err = os.WriteFile(filepath.Join(p.scConfig.StorePath, metricFilename), content, 0600)
	if err != nil {
...
func (p *PTPEventManager) TriggerLogs(url string) error {
