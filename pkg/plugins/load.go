...
// LoadAMQPPlugin ...
func (pl PluginLoader) LoadAMQPPlugin(wg *sync.WaitGroup, amqpHost string, dataIn <-chan *channel.DataChan, dataOut chan<- *channel.DataChan, close <-chan bool) error {
...
//LoadRestPlugin ...
func (pl PluginLoader) LoadRestPlugin(wg *sync.WaitGroup, port int, apiPath, storePath string, eventOutCh chan<- *channel.DataChan, close <-chan bool) (*restapi.Server, error) {
